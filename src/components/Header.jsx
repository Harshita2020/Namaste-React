...
const Header = () => {
  ...
  const handleLoginBtnNameToggle = () => {
    setLoginBtnName((prev) => {
      const newName = prev === "Login" ? "Logout" : "Login";
      setLoggedIn(newName === "Logout"); // true if logging out
      return newName;
    });
  };
    ...
    <div className="flex justify-between bg-pink-50 shadow-lg  w-full z-10 ">
      ...
      <div className="flex items-center">
        <ul className="flex p-4 m-4">
          ...
          {/* className="navul" */}
          <li className="px-4">
            Online Status: {onlineStatus === isLoggedIn ? "✅" : "🔴"}
          </li>
          ...
          <button
            className="px-4 w-20 cursor-pointer hover:bg-amber-50 rounded-lg"
            onClick={() => handleLoginBtnNameToggle()}
          >
            {loginBtnName}
          </button>
